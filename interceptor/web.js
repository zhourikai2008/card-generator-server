...
exports.filter = function(request, response, next) {
    const _str_render = request.url;
    if (
        _str_render == '/api' || _str_render.indexOf('/api/') == 0 || _str_render.indexOf('/api?') >= 0
        ...
        || _str_render.indexOf('/file') == 0 || _str_render.indexOf('/image') == 0
    ) {
        next();
        return;
    }
